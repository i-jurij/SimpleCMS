...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(table) {
        table.addEventListener('click', function(element){
            let form_data = '<form action="" method="post" id="signup_edit_form" name="theForm"><?php echo csrf_field(); ?>\
            ...
            if (element.target.nodeName == 'BUTTON') {
                ...
                if (signup_action == 'delete' && id) {
                    if (document.theForm) {
                        document.theForm.action = '<?php echo e(url()->route("client.signup.remove")); ?>';
                        document.theForm.submit();
                    }
                }
                if (signup_action == 'change' && id) {
                    if (document.theForm) {
                        document.theForm.action = '<?php echo e(url()->route("client.signup.edit")); ?>';
                        document.theForm.submit();
                    }
                }
            }
    ...
    if (edit_div) {
        ...
        function time_change(order_id, serv_dur, master_id) {
            ...
            appointment('short', '/signup/appoint_time', arr.service_id, master_id, '<?php echo e(csrf_token()); ?>');
            ...
            but.addEventListener('click', function (ev) {
                ...
                if ( time_checked ) {
                    ...
                    data_from_db('<?php echo e(url()->route("client.signup.store")); ?>', "order_id=" + order_id+"&serv_dur="+serv_dur+"&start_dt="+start_dt)
        ...
        function master_change(order_id, service_id, start_dt) {
            ...
            data_from_db("<?php echo e(url()->route('client.signup.get_masters')); ?>", "order_id=" + order_id+"&service_id="+service_id+"&start_dt="+start_dt)
            .then(promise => promise)
            .then(masters_data => {
                    ...
                    if (typeof masters_data === 'string' || masters_data instanceof String) {
                        ...
                        document.querySelectorAll('.radio').forEach(function(master_article) {
                            master_article.addEventListener('click', function(){
                                ...
                                if (!!but) {
                                    but.addEventListener('click', function (ev) {
                                        data_from_db('<?php echo e(url()->route("client.signup.store")); ?>', "order_id=" + order_id+"&master_id="+master_id)
                                            .then(promise => promise)
                                            .then(data => {
                                                //location.reload();
...
<?php echo $__env->make("layouts/index", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/client_manikur/client_pages/signup_edit.blade.php ENDPATH**/ ?>