...
<link rel="stylesheet" href="<?php echo e(url()->asset('storage'.DIRECTORY_SEPARATOR.'ppntmt'.DIRECTORY_SEPARATOR.'appointment'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR.'style.css')); ?>" />
    ...
    <?php if(!empty(session('res'))): ?>
        <?php if(is_array(session('res'))): ?>
            <div class="content">
                <h3 class="pad"><?php echo e(session('res')['client_name']); ?></h3>
                <p><b>Вы записались на:</b></p>
                <div class="table_body" style="border-collapse: collapse;">
                    <div class="table_row">
                        <div class="table_cell" style="text-align:right;"><?php echo e(session('res')['service']); ?></div>
                        <div class="table_cell"><?php echo e(session('res')['price']); ?> руб.</div>
                    </div>
                    <?php if(!empty(session('res')['master'])): ?>
                    <div class="table_row">
                        <div class="table_cell" style="text-align:right;">Мастер: </div>
                        <div class="table_cell"><?php echo e(session('res')['master']); ?></div>
                    </div>
                    <?php endif; ?>
                    <div class="table_row">
                        <div class="table_cell" style="text-align:right;">Дата,<br /> время:</div>
                        <div class="table_cell"><?php echo e(session('res')['time']); ?></div>
                    </div>
                    <div class="table_row">
                        <div class="table_cell" style="text-align:right;">Ваш номер:</div>
                        <div class="table_cell"><?php echo e(session('res')['client_phone']); ?> </div>
                    </div>
                    <?php if(!empty(session('res')['client_password'])): ?>
                    <div class="table_row">
                        <div class="table_cell" style="text-align:right;">Ваш пароль (запомните его, если хотите управлять своими записями):</div>
                        <div class="table_cell"><?php echo e(session('res')['client_password']); ?> </div>
                    </div>
                    <?php endif; ?>
                </div>
                <h3>Спасибо за ваш выбор!</h3>
            </div>
        <?php elseif(is_string(session('res'))): ?>
            <div class="content"><p class="error pad"><?php echo e(session('res')); ?></p></div>
        <?php elseif(session('res') === false): ?>
            <p class="error">
                Warning!<br>
                Data of order have been NOT stored!<br>
                You may have already signed up?
            </p>
            <?php echo $dismiss_signup; ?>

        <?php endif; ?>
    <?php else: ?>
        <?php if(!empty(session('dismiss'))): ?>
            <script type="text/javascript">
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    modal_alert("<?php echo e(session('dismiss')); ?>");
                });
            </script>
        <?php endif; ?>
        <div class="content">
            ...
            <form method="post" action="" id="zapis_usluga_form" class="form_zapis_usluga">
                        ...
                        <div class="form-group pad margin_bottom_1rem">
                            <div class="">
                                ...
                                <label class="zapis_usluga">
                                    ...
                                    <input type="text" title="Ваше имя (одно слово, только буквы)" placeholder="Не обязательно заполнять" pattern="^([а-яА-ЯёЁa-zA-Z]+)?$" name="zapis_name" id="zapis_name" maxlength="255" value="<?php echo e(old('name')); ?>" />
                                </label>
                                <br>
                ...
                <div class="choice display_none" id="services_choice">
                    ...
                    <div class="zapis_usluga page_buttons">
                        <?php $__currentLoopData = $data['serv']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $cat_arr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <button type="button" class="buttons zapis_usluga_buttons masters_edit vertaligntop" id="<?php echo e(translit_to_lat(sanitize($page))); ?>">
                                <img class="" src="<?php echo e(asset('storage'.DIRECTORY_SEPARATOR.$data[$page])); ?>" alt="<?php echo e($page); ?> image" />
                                <?php echo e($page); ?>
                    ...
                    <div class="zapis_usluga zapis_usluga_spisok">
                        <?php $__currentLoopData = $data['serv']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $cat_arr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="uslugi display_none" id="div<?php echo e(translit_to_lat(sanitize($page))); ?>" >
                                <?php $__currentLoopData = $cat_arr; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat_name => $serv_arr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="">
                                    <?php if($cat_name !== 'page_serv'): ?>
                                        <?php $__currentLoopData = $serv_arr; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $serv_name => $serv_duration): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                                $id = translit_to_lat(sanitize($serv_name)).'plus'.$serv_duration;
                                                list($price, $duration, $serv_id) = explode('-', $serv_duration);
                                            ?>
                                            <label class="custom-checkbox back text_left" for="<?php echo e($id); ?>">
                                                <input
                                                    type="radio"
                                                    name="usluga"
                                                    value="<?php echo e($serv_id); ?>"
                                                    id="<?php echo e($id); ?>"
                                                />
                                                <span><?php echo e($page); ?>,<br><?php echo e($cat_name); ?>: <?php echo e($serv_name); ?>,<br><?php echo e($price); ?> руб.</span>
                                            </label>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php elseif($cat_name == 'page_serv'): ?>
                                        <?php $__currentLoopData = $serv_arr; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $serv_name => $serv_duration): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                            ...
                                            <label class="custom-checkbox back text_left" for="<?php echo e($id); ?>">
                                                <input
                                                    ...
                                                    id="<?php echo e($id); ?>"
                                                />
                                                <span><?php echo e($page); ?>,<br><?php echo e($serv_name); ?>,<br><?php echo e($price); ?> руб.</span>
                                            </label>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php endif; ?>
...
$(function() {
        ...
        dismiss_order.click(function() {
            ...
            if (res.client_name && res.client_phone && password ) {
                ...
                $('form#zapis_usluga_form')
                    .attr('action', '<?php echo e(url()->route("client.signup.list")); ?>')
                            ...
                            if (element.data_uvoln == '' || element.data_uvoln == null) {
                                mst += '<article class="main_section_article radio" data-value="'+element.id+'">\
                                    <div class="main_section_article_imgdiv" style="background-color: var(--bgcolor-content);">\
                                        <img src="<?php echo e(asset("storage")); ?>/'+element.master_photo+'" alt="Фото '+element.master_fam+'" class="main_section_article_imgdiv_img" />\
                                    </div>\
    ...
    else if ( ( $('#master_choice #master').val() || $('#master_choice').html() == '<p class="pad">No masters for this service available.</p>') && $(this).val() == 'time_choice' )
    {
        ...
        appointment('short', '/signup/time', service, master, "<?php echo e(csrf_token()); ?>");
        $('html, body').animate({
    		...
    		success: function(responce){
                if (responce.hasOwnProperty('res') && responce.res == true) {
                    ...
                    if (responce.hasOwnProperty('master_data')) {
                        ...
                        mp = ' <img \
                                    src="<?php echo e(asset("storage")); ?>/'+master_photo+'" \
                                    alt="Photo '+master+'" \
                    ...
                    let dismiss_signup = "<?php echo $dismiss_signup; ?>";
    ...
    } else if ( $(this).val() == 'zapis_sql' )
    {
        ...
        $('form#zapis_usluga_form').attr('action', '<?php echo e(url()->route("client.signup.end")); ?>');
        /*
        $.ajax({
...
<?php echo $__env->make("layouts/index", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/client_manikur/client_pages/signup.blade.php ENDPATH**/ ?>