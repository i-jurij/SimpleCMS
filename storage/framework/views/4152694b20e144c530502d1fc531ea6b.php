    ...
    <?php if(!empty($data)): ?>
        <?php if(is_array($data)): ?>
            <?php if(!empty($data['by_date'])): ?>
                ...
                echo $res;
                ?>
            <?php elseif(!empty($data['by_master'])): ?>
                <div id="app">
                    <p class="pad">Выберите мастера:</p>
                <?php $__currentLoopData = $data['by_master']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $master): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    ...
                    <button
                        ...
                        id="<?php echo e($master['id']); ?>"><?php echo $master_name; ?></button>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            <?php elseif(!empty($data['list']) || !empty($data['post_by_client'])): ?>
                <div id="list_wrapper">
                    <div id="js_table"></div>
...
<script>
    window.onload = function() {
        ...
        function time_change(order_id, serv_dur, master_id) {
            ...
            but.addEventListener('click', function (ev) {
                ...
                if ( time_checked ) {
                    ...
                    data_from_db("<?php echo e(url()->route('admin.signup.edit.post')); ?>", "order_id=" + order_id+"&serv_dur="+serv_dur+"&start_dt="+start_dt)
        ...
        function master_change(order_id, service_id, start_dt) {
            ...
            data_from_db("<?php echo e(url()->route('admin.signup.get_masters')); ?>", "order_id=" + order_id+"&service_id="+service_id+"&start_dt="+start_dt)
            .then(promise => promise)
            .then(masters_data => {
                    ...
                    if (typeof masters_data === 'string' || masters_data instanceof String) {
                        ...
                        for (const master of masters_data) {
                            document.querySelector('#master_choice').innerHTML += '<article class="main_section_article radio" data-value="'+master.id+'" style="min-width:13rem;">\
                                    <div class="main_section_article_imgdiv" style="background-color: var(--bgcolor-content);">\
                                        <img src="<?php echo e(asset("storage")); ?>/'+master.master_photo+'" alt="Фото '+master.master_fam+'" class="main_section_article_imgdiv_img" />\
                                    </div>\
                        ...
                        document.querySelectorAll('.radio').forEach(function(master_article) {
                            master_article.addEventListener('click', function(){
                                ...
                                if (!!but) {
                                    but.addEventListener('click', function (ev) {
                                        data_from_db("<?php echo e(url()->route('admin.signup.edit.post')); ?>", "order_id=" + order_id+"&master_id="+master_id)
                                            .then(promise => promise)
                                            .then(data => {
                                                //location.reload();
        ...
        function fetch_del_change(element) {
            if (element.target.nodeName == 'BUTTON') {
                        ...
                        if (action == 'delete' && id !== 'undefined') {
                            data_from_db("<?php echo e(url()->route('admin.signup.remove')); ?>", "order_id=" + id)
                            .then(promise => promise)
                            .then(master_data => {
                                location.reload();
                            })
                        ...
                        if (action == 'change' && id !== 'undefined') {
                            history.pushState(document.body.innerHTML, "", '<?php echo e(url()->current()); ?>#change');

                            let div = document.querySelector('#js_edit');
                            //3 buttons for choice time or master or service
                            data_from_db("<?php echo e(url()->route('admin.signup.edit')); ?>", "order_id=" + id)
                            .then(promise => promise)
                            .then(master_data => {
                                    ...
                                    if (!!table_ch) {
                                        table_ch.addEventListener('click', function(element){
                                            ...
                                            if (element.target.id == 'time_ch') {
                                                //history.pushState(document.body.innerHTML, "", '<?php echo e(url()->current()); ?>#time_change');
                                            ...
                                            if (element.target.id == 'master_ch') {
                                                //history.pushState(document.body.innerHTML, "", '<?php echo e(url()->current()); ?>#master_change');
        ...
        for (var i = 0; i < elem.length; i++) {
            elem[i].onclick = function (e) {
                ...
                //history.pushState(document.body.innerHTML, "", '<?php echo e(url()->current()); ?>');
                document.querySelector('#app').innerHTML = '';
                //document.querySelector('#div_'+id).toggle();
                data_from_db("<?php echo e(url()->route('admin.signup.post_by_master')); ?>", "master_id=" + id)
                    .then(promise => promise)
                    .then(master_data => {
                        let string = '';
        ...
        if (!!js_table) {
            ...
            //history.pushState('', "", '<?php echo e(url()->current()); ?>#list');
            if ( !'<?php echo e(url()->current()); ?>'.includes(window.location.hash)) {
                //history.pushState('', "", "<?php echo e(url()->route('admin.home')); ?>");
                //history.pushState('', "", '<?php echo e(url()->current()); ?>');
            }
...
<?php echo $__env->make("layouts/index_admin", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/admin_manikur/moder_pages/signup.blade.php ENDPATH**/ ?>